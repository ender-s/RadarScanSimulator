...
public class CircularScan extends Scan 
{
    
    private CircularMotion circularMotion;
    private Cone cone;
    
    public CircularScan(ScanGraphWindow windowReference, Antenna antenna, double step,
    ...
    private double getAzimuthAtAngle(double teta)
    {
        ...
        azimuthOfBeam = getAzimuthByPosition(beam);
        azimuthOfTarget = getAzimuthByPosition(targetPosition);
        
        double azimuth = Math.abs(azimuthOfBeam - azimuthOfTarget);
        if (azimuth > 180) azimuth = 360 - azimuth;
        
        if (HelperMethods.DEBUG)
            System.out.printf("Calculating azimuth at angle %f | Beam: (%f, %f, %f) azimuthOfBeam: %f | Target: (%f, %f, %f) azimuthOfTarget: %f | Azimuth: %f%n", 
                teta, beam.getX(), beam.getY(), beam.getZ(), azimuthOfBeam, targetPosition.getX(),
                targetPosition.getY(), targetPosition.getZ(), azimuthOfTarget, azimuth);
    ...
    public boolean fetchValues()
    {
        ArrayList<String> fieldNamesArrayList = new ArrayList<String>(Arrays.asList("Target Range",
            "Elevation Angle",
            "Degree of Target (Azimuth)",
            "Period of Circular Motion"));

        javax.swing.JComponent[] targetMovementComponents = windowReference.getCircularTargetMovementComponents();      
        
        ArrayList<javax.swing.JTextField> textFieldArrayList = 
                new ArrayList<javax.swing.JTextField>(windowReference.getSpecialTextFields().subList(4, 8));
        ...
        double elevationAngle = Double.parseDouble(textFields[1].getText());
        ...
        double h, r, x = targetRange;
        double alpha = elevationAngle;
        if (elevationAngle > 90) alpha -= 90;
        r = x * Math.sin(Math.toRadians(alpha));
        h = x * Math.cos(Math.toRadians(alpha));
        
        this.cone = new Cone(h, alpha);
        
        
        System.out.printf("Radius of circular motion is %f\n", r);
        
        this.circularMotion = new CircularMotion(r, periodOfCircularMotion, 0, true);
